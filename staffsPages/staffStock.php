        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped">
				...
				<tbody>
					<?php 
						$i = 1;
						$qry = $conn->query("SELECT * from `requests` Where `appByStore`=1 AND `appByReq`=1 AND `requestedBy`='$id';");
						if($qry->num_rows > 0){
              ...
              <!-- <a href="./updateStock.php?id=<?php echo $row['id']?>" class="dropdown-item btn btn-primary" ><span class="d-inline-block bg-success rounded-circle p-1"></span> Edit</a>
				...
				<a  href="./addingReturn.php?id=<?php echo $row['id']?>" class="dropdown-item btn btn-secondary"><span class="d-inline-block bg-secondary rounded-circle p-1"></span> Return stock</a> -->
					...
					<?php endwhile; }
						...
						echo '<h2>Sorry, No Available stock please purchase a new stock!<h2/><a href="./staffRequests.php" style="align-self: flex-end;" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Go to Requesting page?</a>
						';
					}?>
				</tbody>
			</table>
